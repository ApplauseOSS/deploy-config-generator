...
Kubernetes deployment output plugin

### Parameters


#### Deploy config section: kube_deployments
...
`spec . paused`|`bool`|no||
`spec . progress_deadline_seconds`|`int`|no||
...
`spec . strategy`|`dict`|no||
`spec . strategy . rolling_update`|`dict`|no||
`spec . strategy . rolling_update . max_surge`|`str`|no||
`spec . strategy . rolling_update . max_unavailable`|`str`|no||
`spec . strategy . type`|`str`|yes||
